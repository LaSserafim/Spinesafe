...
The application is built using a modern React 18 stack, designed for high performance and an elegant, premium, dark-mode research laboratory aesthetic. It leverages **Three.js** and **React Three Fiber** to render a rigged, animatable human model (`untitled.glb`), allowing users to visually manipulate the model's skeletal posture via a UI slider and instantly see the physical forces applied.
...
- **Ultimate Case-Insensitive Bone Search**: Resolves skeleton bone names (e.g. `Head`, `Spine1`, `LeftShoulder`) regardless of naming prefixes, suffixes, colons, case variations, or Blender export quirks.
- **Premium Glassmorphism UI**: Uses glass-panel containers (backdrop-filter blur, subtle borders, soft shadows) sitting over a deep, radial lab gradient background.
- **Fluid Scroll Animations**: Implements Framer Motion slide-up and fade-in effects triggered on viewport entry.
...
- **Styling**: Vanilla CSS (CSS Variables, Backdrop-filters, Glassmorphism)
...
* `vercel.json` - Custom Vercel deployment routes utilizing clean, regex-free path-to-regexp paths.

### Application Logic & UI
* `src/main.tsx` - The React initialization script that mounts the application to the DOM.
* `src/App.tsx` - The primary state manager and layout container. It manages the slider input, calculates physics values (Torque, Spinal Load, Risk Assessment), renders the Chart.js graph, manages the Report Generator form, and coordinates the 3D scenes with Framer Motion triggers.
* `src/index.css` - The global design system, including deep dark lab gradient backgrounds, typography (Space Grotesk & Inter), layout grids, and custom `.glass-panel` class styles.
...
* `src/components/HeroScene.tsx` - An alternative canvas configuration.
* `src/components/3d/HumanModel.tsx` - The main 3D component. It imports the GLB, corrects skeleton offsets dynamically, enforces a natural resting pose, and applies slider-driven skeletal deformations via quaternion spherical linear interpolation (`slerp`).
